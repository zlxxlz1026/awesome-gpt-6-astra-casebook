...
This guide is editorial analysis of the collected cases. Project recommendations are clearly separated from the creators' public prompts.
...
## Requirement checklist for an interactive exhibit

Use the public case prompts above as references, then decide these requirements for your own project:

- Audience and purpose: what the visitor should see, learn or control.
- Delivery: source code, startup instructions, dependencies and known constraints.
- Scene: the main object, background and supplied or procedural assets.
- Interaction: rotation, zoom, selection, reset and touch equivalents where relevant.
- Feedback: labels or explanations shown after each interaction.
- Responsive layout: how the scene and controls adapt on narrow screens.
- Sources: external dependencies, asset origins and factual references.
- Acceptance: startup, rotation, zoom, selection, reset and responsive behavior.
...
3. Narrow the window, then use a real touch device when touch support is in scope. Record a touch result only when the corresponding device was used.
...
- Case facts remain attributed to the catalog's linked [peacock post](https://x.com/lepadphone/status/2096147245775331419), [playroom post](https://x.com/zhengli/status/2096048421543272893) and [V8 post](https://x.com/DilumSanjaya/status/2096280244663775423).
